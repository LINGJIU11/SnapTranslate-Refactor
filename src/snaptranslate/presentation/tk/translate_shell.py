...
def _create_variables(host: ShellHost, root: tk.Tk) -> None:
    ...
    proxy_mode, proxy_url = host.proxy_default()
    host.proxy_mode_var = tk.StringVar(master=root, value=proxy_mode)
    host.proxy_url_var = tk.StringVar(master=root, value=proxy_url)
