...
### N5 控制台"显示已打开其实打不开 / 关不掉" ← 已改（第二版实测反馈）
- **现象**（第二版实测）：双击 exe 后控制台就显示"划词翻译已经打开"，但此时它并没真的可用；
  既没法翻译，也没法关闭或打开划词翻译窗口。
- **实测澄清**（先复现再改）：清场后按 PID 精确测，**冻结产物的窗口 0.6 秒就出来、
  `WM_CLOSE` 也能正常退出**——窗口本身没问题。真正的问题在控制台的"状态判定与开关路径"：
  1. **状态撒谎**：第一版只有"运行中/未运行"两态，"进程刚起来、窗口还没画出来"以及
     "窗口隐藏到托盘"都被显示成"运行中"，于是用户以为开好了却点不动；
  2. **打开不保证成功**：`FindWindowW` 连隐藏窗口也返回，而它只调 `force_foreground`
     （Tk `withdraw()` 的窗口必须 `ShowWindow(SW_SHOW)` 才会重新出现），
     打开失败时还会显示"已唤起"；
  3. **没有出口**：控制台缩到托盘后进程仍在，而控制台自己也占着单实例互斥量
     → 再双击 exe 会"抢不到单实例 + 找不到可见窗口" → 静默退出，用户彻底进不去；
     而且当时没有任何"关闭某个子窗口"的按钮。
- **改法**：
  1. 状态改五档并**以"可见窗口"为准**：`stopped / starting / running / hidden / stuck`
     （进程活着但超 25 秒没窗口算 `stuck`），面板每秒刷新显示；
  2. 打开时先 `ShowWindow(SW_SHOW+SW_RESTORE)` 再置前；**打不开就如实报错**
     （不再假装"已唤起"）；
  3. 每一行加 **「关闭」与「重启」**：句柄在手就 terminate，不在手就**按窗口标题找到 PID 结束**
     ——不管窗口是怎么起来的都能关掉；`stuck` 时点"打开"会自动先关旧再重开；
  4. **控制台不再抢单实例互斥量**（只按可见窗口判重）：互斥量被"看不见的控制台"占住是死角的根源；
     `--app=X` 第二实例改为**等窗口最多 15 秒**，仍找不到就弹提示告诉用户去控制台关/重启；
  5. **默认不常驻托盘**：不勾选时关窗就是真退出；勾选后缩到托盘会弹气泡说明怎么退出。
- **回归测试**：`tests/test_launcher.py`（25 项，含五档状态、隐藏窗口先显示再置前、
  打不开返回 False、连点不双开、stuck 自动重开、按标题关窗口、重启先关后开）、
  `scripts/gui_smoke.py --with-tk`（面板构造 / 状态词 / 默认不常驻 / 关窗即退出 /
  **真实 Tk + Win32：withdraw 后 is_visible 为假、show_window 后重新可见**）。
- **交付**：`v2.1.2` 第三次打包。
