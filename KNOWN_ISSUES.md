...
**但阶段一之后有两处主动修复**（用户实测后要求，属于有意偏离原版）：取词失败被静默吞掉、
界面改热键要重启才生效——见 §五 `[修复]`。修完仍有回归测试兜底，其余条目状态不变。
...
| D7 | **F1**：取词失败不再静默翻译剪贴板旧内容（改为明确报错并放弃翻译） | 见 §五；行为有意偏离原版，用户实测后要求 |
| D8 | **F2**：界面改热键立即生效（监听器支持热更新） | 见 §五；这本来就是原版行为，阶段一漏掉，属补回 |
